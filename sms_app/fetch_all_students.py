...
class SMSStudentListFetcher:
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            # 调试：打印响应信息
            print(f"[DEBUG] 响应 URL: {resp.url}")
            print(f"[DEBUG] 响应状态码: {resp.status_code}")
            print(f"[DEBUG] 响应长度: {len(resp.text)}")
            
            # 检查是否仍在登入页面 - 改进检查逻辑
            if 'login' in resp.url.lower() or 'username' in resp.text.lower():
                print(f"[ERROR] 登入失败: 返回登入页面或包含登入表单")
                print(f"[DEBUG] 响应内容摘要: {resp.text[:500]}")
                return False
    ...
    def _fetch_class_list(self) -> bool:
        ...
        try:
            ...
            print(f"[DEBUG] 找到 {len(all_selects)} 个 SELECT 元素")
            for i, select in enumerate(all_selects):
                select_id = select.get('id', 'no-id')
                select_name = select.get('name', 'no-name')
                options_count = len(select.find_all('option'))
                print(f"[DEBUG] SELECT {i}: id={select_id}, name={select_name}, options={options_count}")
            
            # 查找班级选择器 SELECT 元素
            class_select = soup.select_one('select#StudentPerformanceM_class_id')
            if not class_select:
                print(f"[WARNING] 未找到 ID 为 StudentPerformanceM_class_id 的 SELECT 元素")
                # 尝试通过其他方式查找班级选择器
                class_select = soup.select_one('select[name*="class"]')
                if not class_select:
                    print(f"[WARNING] 也未找到任何包含 'class' 的 SELECT 元素")
                    return False
                print(f"[DEBUG] 找到可能的班级 SELECT: {class_select.get('id', 'no-id')}")
            
            # 从 SELECT 中提取所有 option
            self.class_mapping = {}
            options = class_select.find_all('option')
            print(f"[DEBUG] 提取 {len(options)} 个班级选项")
            for option in options:
                ...
                if class_id and class_name and class_name != '--- 选择班级 ---':
                    self.class_mapping[class_id] = class_name
                    print(f"[DEBUG] 班级: {class_id} → {class_name}")
...
def main():
    ...
    # 配置
    USERNAME = "schhs334"  # 修改为实际用户名
    PASSWORD = "@Sidan49122"  # 修改为实际密码
