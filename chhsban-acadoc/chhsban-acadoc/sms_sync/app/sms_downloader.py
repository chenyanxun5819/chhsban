...
"""
SMS 学生资料下载工具 - 改进版
使用 requests 和 BeautifulSoup 从正确的位置获取有效班级
不需要浏览器驱动，更轻量级和可靠
"""
...
class SMSStudentDownloader:
    ...
    def _get_valid_classes(self) -> Dict[str, str]:
        ...
        获取 XPath 对应位置的 select 元素中的班级列表
        ...
        try:
            ...
            # 策略1：使用指定 XPath 对应的 CSS 选择器查找 select
            # XPath 的深层 select 通常可以通过多种 CSS 选择器访问
            classes = {}
            ...
            for idx, select in enumerate(all_selects):
                ...
                # 检查这个 select 是否包含班级数据（至少有10个以上的选项）
                if len(options) > 5:
                    temp_classes = {}
            ...
            if not classes:
                self.log("  未找到班级列表，尝试备用选择器...", "WARN")
                # 备用策略：查找 name="class_id" 的 select
